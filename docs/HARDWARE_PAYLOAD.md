...
| Volumetric accuracy | Not specified as ground truth — repeatability is the operative figure |
| Payload capacity | ~500 g per arm |
| Programming | RobotStudio (offline trajectory + playback) |
| Controller | IRC5 |
| ROS interface | `abb_robot_driver` (ROS 2) |

**Important:** The ±0.02 mm figure is **repeatability** (per ABB product specification; see RESEARCH_PLAN References [16]), not volumetric accuracy. Ground truth confidence is bounded by repeatability, not by calibrated absolute accuracy. This distinction must be explicit in publications.
...
| B (RealSense D455) | ~275 g | 55% | ✅ Safe |
| C (RPLiDAR A2M12) | ~250 g | 50% | ✅ Safe |
| D (Livox Mid-360) | ~350 g | 70% | ⚠️ At margin — verify exact weight |

> **Action required:** Weigh all sensors + mounts on a physical scale before finalizing Session D scheduling. If the Mid-360 assembly exceeds 400 g, redesign mount to reduce weight.
...
**Notes:** Session C is 2D-only (Z fixed). The A2M12 provides a planar SLAM baseline established in the literature and commonly used in 2D benchmarks, with no geometric degeneracy risk in properly structured environments. It serves as the minimal-complexity comparison baseline before evaluating the 3D sensors.
...
|---------|--------|-------|------------|----------|-------|
| A | 50 g | 50 g | **100 g** | ❌ | Simplest mount; low risk |
| B | 195 g | 80 g | **275 g** | ❌ | Camera mount needs cable management |
| C | 170 g | 80 g | **250 g** | ❌ | 2D only; Z constraint critical |
| D | 250 g | 100 g | **350 g** | ❌ | At 70% capacity; priority to verify |
...
**When to invoke the Mitsubishi option:** If the YuMi payload is definitively exceeded for a specific session (most likely Session D if Mid-360 + mount > 500 g), request access to the Mitsubishi as fallback. Document the different ground truth source in the experimental record.
...
| LED panel + constant-current driver | Stable illumination for Session B | 30 min warm-up before session start |
